import { ALPHABET_CHARS } from "./constants";
...
export const getManufacturers = () => {
    ...
    for (let i = 0; i < manufacturers.length; i = i + 2) {
        const code = manufacturers[i];
        const manufacturer = manufacturers[i + 1];

        const codes = fill(code);
        codes.forEach(c => theMap[c] = manufacturer);
    }
    return theMap;
};
...
const fill = (code:string) : string[] => {
    if (code.length === 3) {
        ...
        for (let i = 0; i < ALPHABET_CHARS.length; i++) {
            codes.push(code + ALPHABET_CHARS[i]);
        }
        return codes;
    } else if (code.length > 3) {
        return fill(code.substring(0, 3));
    } else {
        throw new Error("Invalid code");
    }
};

export const getManufacturer = (code:string) => {
    if (code.length === 3) {
        return getManufacturers()[code];
    } else {
        throw new Error("Invalid code");
    }
};
